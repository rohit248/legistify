    ...
    public function sendmail()
    {
    	$postData = $this->input->post();
    	 ...
    	 $this->load->library('email');
                ...
                $config['smtp_user'] = 'rohit.yadav@hotmail.co.in';
                $config['smtp_pass'] = 'CtFKTeNNVB0JQWNjFKJQ9g';
                ...
                $this->email->from('no-reply@please.com','Made By Rohit Yadav');
                $this->email->to($postData['hidemail']); 
...
else
{
    $file_name = $this->upload->file_name;
     ...
     $this->email->subject('Answer to your query');
                $this->email->message($postData['msg'].'.Follow the link for attachment      '.'http://localhost/hmvc-legistify/legistify/legistify/application/modules/home/views/file_upload/'.$file_name);    
                 $id=$postData['hideid'];
                $this->main_model->deleteQuery('queries', array('id' => $id)); 
                $this->email->send();
                
                echo "Mail Has Been Sent Successfully....!!!!Go Back to Reply to another query!!!!!"; 
